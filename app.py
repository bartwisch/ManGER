...
def smooth_polygon_for_display(
    polygon: list[tuple[int, int]], num_points: int = 100
) -> list[tuple[int, int]]:
...
def draw_text_boxes(
    image: Image.Image, blocks: list, show_text: bool = True, show_polygons: bool = False
) -> Image.Image:
    ...
    for i, block in enumerate(blocks):
        ...
        if show_text and block.original_text:
            # Truncate long text
            text_preview = (
                block.original_text[:20] + "..."
                if len(block.original_text) > 20
        ...
        draw.rectangle(
            [text_bbox[0] - 2, text_bbox[1] - 2, text_bbox[2] + 2, text_bbox[3] + 2], fill=color
        )
...
def get_pipeline(settings: dict | None = None) -> MangaPipeline:
    ...
    if settings:
        ...
        if (
            settings.get("api_key") != current_settings.get("api_key")
            or settings.get("provider") != current_settings.get("provider")
            or settings.get("source_lang") != current_settings.get("source_lang")
            or settings.get("target_lang") != current_settings.get("target_lang")
            or settings.get("ocr_engine") != current_settings.get("ocr_engine")
            or settings.get("magi_version") != current_settings.get("magi_version")
        ):
            needs_recreate = True
    ...
    if st.session_state.pipeline is None or needs_recreate:
        ...
        if MAGI_AVAILABLE:
            ocr_service = MagiOCRService(
                config.ocr, use_manga_ocr=use_manga_ocr, model_version=magi_version
            )
        else:
...
def render_sidebar():
    ...
    source_lang = st.sidebar.selectbox(
        ...
        options=["en", "ja", "ko", "zh"],
        index=0,
        format_func=lambda x: {"en": "English", "ja": "Japanese", "ko": "Korean", "zh": "Chinese"}[
            x
        ],
    )
...
def _process_all_selected_pages(settings: dict):
    ...
    for idx, page_num in enumerate(selected_pages):
        progress_bar.progress(
            (idx) / total, text=f"Processing page {page_num + 1} ({idx + 1}/{total})..."
        )
...
def _show_download_dialog():
    ...
    export_format = st.radio(
        ...
        index=0,
        horizontal=True,
        help="PDF: Universal format. CBZ: Comic Book Archive (recommended for manga readers)",
    )
    ...
    if export_format == "CBZ":
        cbz_col1, cbz_col2 = st.columns(2)
        with cbz_col1:
            image_format = st.selectbox(
                "Image Format",
                options=["JPEG", "WEBP", "PNG"],
                index=0,
                help="JPEG: Good compression, widely supported. WEBP: Best compression. PNG: Lossless.",
            )
        with cbz_col2:
            cbz_quality = st.slider(
                "Quality",
                min_value=50,
                max_value=100,
                value=85,
    ...
    if processed_pages:
        min_page = min(processed_pages) + 1
    ...
    if export_format == "PDF" and not file_name.endswith(".pdf"):
        file_name += ".pdf"
    elif export_format == "CBZ" and not file_name.endswith(".cbz"):
        file_name += ".cbz"
    ...
    with col1:
        ...
        try:
            if export_format == "PDF":
                file_bytes = st.session_state.pdf_service.create_pdf_from_images(
                    result_images,
                    quality=pdf_quality,
                ...
                file_bytes = archive_service.create_cbz(result_images)
            ...
            st.error(f"Failed to create {export_format}: {e}")
...
def render_page_selector():
    ...
    with col_mode:
        range_mode = st.toggle(
            "Range Selection Mode",
            value=st.session_state.range_select_mode,
            help="When ON, clicking a page selects all pages between it and the last clicked page.",
        )
    ...
    with col_btns:
        ...
        with btn2:
            if st.button(
                "Clear Selection", use_container_width=True, on_click=dismiss_download_dialog
            ):
                st.session_state.selected_pages = []
                for i in range(page_count):
        ...
        for row_start in range(0, len(thumbnails), cols_per_row):
            ...
            for col_idx, (page_num, thumb) in enumerate(
                ...
                with cols[col_idx]:
                    ...
                    thumb_display = thumb_display.resize(
                    ...
                    if thumb_display.width > 120:
                        thumb_display = thumb_display.resize(
                            (120, int(120 / aspect)), Image.Resampling.LANCZOS
                        )
                        y_offset = (150 - thumb_display.height) // 2
                        x_offset = 0
...
def render_page_viewer():
    ...
    with nav_col4:
        if st.button(
            "Next ▶️",
            disabled=current_idx >= len(selected) - 1,
            use_container_width=True,
            on_click=dismiss_download_dialog,
        ):
            st.session_state.current_page_idx = current_idx + 1
    ...
    with col1:
        ...
        if page_data["blocks"]:
            img_with_boxes = draw_text_boxes(
                page_data["image"], page_data["blocks"], show_polygons=show_polygons
            )
...
def process_all_pages():
    ...
    if provider == "openai" and not api_key:
        st.error(
            "⚠️ OpenAI API key required. Enter it in the sidebar or switch to 'dummy' provider."
        )
        return
